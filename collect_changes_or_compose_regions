...
class collectChangesCommand(sublime_plugin.TextCommand):
    def run(self, edit, addArbitraryRegions=False):
        ...
        if addArbitraryRegions:
            # This block of code is a quick-and-dirty way of adding these 2 minor features
            clpbrd = sublime.get_clipboard()
            if clpbrd == 'select myCollection':
                self.view.sel().clear(); self.view.sel().add_all(my_collection_list)
        ...
        def on_highlight(index):
            self.view.add_regions('focusedRegions', [my_collection_list[index]], "string", "dot")
            self.view.show_at_center(my_collection_list[index].a)
        ...
        def on_select(index):
            if index == -1: # Cancel operation
                clean_up()
                return
            yesno = sublime.yes_no_cancel_dialog("[Exit] to close the panel\n\n[Exclude] if you want to exclude " + str(my_collection_list[index]) + " from the collection of regions\n\n[Cancel] to dismiss this dialog and stay with the panel", "Exit", "Exclude")
            if yesno == sublime.DIALOG_YES: # to Exit
                clean_up()
                self.view.sel().clear(); self.view.sel().add_all(my_collection_list) # Select all the regions
                return
            elif yesno == sublime.DIALOG_NO: # to Exclude
                my_collection_list.pop(index)
                self.view.erase_regions('myCollection')
                self.view.add_regions('myCollection', my_collection_list)
                my_quickpanel(index)
        ...
        def prepare_the_show():
            self.view.erase_regions('showScope')
            self.view.add_regions('showScope', my_collection_list, "string", "", sublime.DRAW_NO_FILL)
            my_quickpanel_list = []
            for region in my_collection_list:
                row, col = self.view.rowcol(region.a)
                text = self.view.substr(region)
                text_to_show = text if len(text) < 66 else text[0:62] + "..."
                my_quickpanel_list.append(str(region) + " at Line " + str(row+1) + " Pos " + str(col+1) + ": " + text_to_show)
            return my_quickpanel_list

        # Function to show the quick panel
        def my_quickpanel(index, reload=True):
            if reload: my_quickpanel_list = prepare_the_show()
            self.view.window().show_quick_panel(my_quickpanel_list, on_select, 1, index, on_highlight)
